...
class ContentCardTemplateTests(TestCase):
    def test_local_content_item_with_title_renders_without_name_attribute(self):
        ...
        rendered = render_to_string('partials/card.html', {'item': item, 'watchlist_ids': []})
    ...
    def test_direct_availability_is_primary_watch_action(self):
        ...
        ContentAvailability.objects.create(content=item, provider='PBS', url=item.url, access_type='free')
        rendered = render_to_string('partials/card.html', {'item': item, 'watchlist_ids': []})
        self.assertIn('Watch on PBS', rendered)
        self.assertNotIn('Open source', rendered)

    def test_tmdb_discovery_loads_regional_compact_watch_context(self):
        ...
        self.assertIn('data-region="US"', rendered)
        self.assertIn('data-require-region="1"', rendered)
