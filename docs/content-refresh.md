...
- **TVmaze — On TV Today:** fetched on each page load for the selected account region. Up to 100 unique scheduled shows are considered before account discovery preferences are applied. Shows are retained even when TVmaze does not publish an `officialSite`, so real network, episode, runtime, airtime, genre, and image data are not discarded. When an official destination is unavailable, aitv labels the card accordingly and exposes TVmaze source details instead of inventing a watch link. Metadata-only TVmaze cards are lazily matched to TMDB by exact normalized title as they approach the viewport, using the TVmaze premiere year as an additional disambiguation signal when available. If TMDB has concrete providers in the selected region, aitv adds those provider labels and a **See regional watch options** action; no provider URL is guessed. Matching/provider results are cached for 30 minutes so scrolling does not repeatedly hit TMDB. In strict regional mode, metadata-only TVmaze cards stay hidden while this enrichment runs, are revealed only after concrete regional availability is confirmed, and are removed when no safe match/provider is found or validation fails. Cards with a real source-supplied watch destination remain ranked ahead of metadata-only cards; personalized genre relevance still orders titles within the same actionability group.
