...
- Notification read actions are per-user and POST-only. Marking an individual notification read returns to the current inbox page when the submitted return URL is same-site; unsafe/external return targets fall back to the inbox.
