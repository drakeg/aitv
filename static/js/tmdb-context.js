document.addEventListener('DOMContentLoaded', () => {
  ...
  const orderedProviders = (rows) => {
    ...
    return (rows || []).map((provider, index) => ({provider, index})).sort((left, right) => {
      ...
      if (leftPreferred && leftRank !== rightRank) return leftRank - rightRank;
  ...
  const enrichTmdbContext = async (element) => {
    ...
    try {
      ...
      providers.replaceChildren(); addMutedPill(providers, 'Watch details temporarily unavailable');
    }
  };
  ...
  const enrichTvmazeCard = async (card) => {
    ...
    try {
      ...
      if (metadataOnly && data.is_available_in_region && providerBox) {
        providerBox.replaceChildren(); renderProviders(providerBox, data);
        if (data.watch_url && !card.querySelector('[data-tvmaze-regional-watch]')) {
          const action = document.createElement('a'); action.className = 'btn btn-sm btn-primary w-100 mt-1'; action.target = '_blank'; action.rel = 'noopener noreferrer';
          action.href = data.watch_url; action.textContent = watchActionLabel(data); action.dataset.tvmazeRegionalWatch = ''; detailsLink.before(action);
        }
      }
  ...
  function renderTvmazeSaveControls(card, data) {
    ...
    if (data.saved && data.remove_url) {
      form.action = data.remove_url; form.dataset.contentId = data.content_id || ''; appendCsrf(form); form.appendChild(makeButton('✓ Saved to Watchlist', 'btn-danger')); container.appendChild(form);
      if (data.favorite_url) {
        const favorite = document.createElement('form'); favorite.method = 'post'; favorite.action = data.favorite_url; favorite.dataset.favoriteForm = ''; favorite.dataset.favoriteState = data.favorite ? '1' : '0';
        appendCsrf(favorite); appendHidden(favorite, 'favorite', data.favorite ? '0' : '1'); favorite.appendChild(makeButton(data.favorite ? '★ Favorite' : '☆ Mark favorite', data.favorite ? 'btn-warning' : 'btn-outline-light')); container.appendChild(favorite);
      }
      return;
    }
    form.action = importUrl; form.dataset.externalSave = 'true'; appendCsrf(form);
    appendHidden(form, 'title', data.tmdb_title || card.querySelector('.content-title')?.getAttribute('title') || 'TV Show'); appendHidden(form, 'url', data.tmdb_details_url);
    appendHidden(form, 'genre', card.querySelector('.source-genre')?.getAttribute('title') || 'TV'); appendHidden(form, 'thumbnail', card.querySelector('img')?.src || '');
    appendHidden(form, 'content_type', 'tv'); appendHidden(form, 'description', ''); appendHidden(form, 'release_year', card.dataset.releaseYear || ''); appendHidden(form, 'rating', '');
    appendHidden(form, 'external_source', 'tmdb'); appendHidden(form, 'external_id', data.tmdb_id); form.appendChild(makeButton('⭐ Save to Watchlist', 'btn-success')); container.appendChild(form);
  }
  ...
  function observeNearViewport(elements, callback) { if (!elements.length) return; if (!('IntersectionObserver' in window)) { elements.forEach(callback); return; } const observer = new IntersectionObserver((entries) => entries.forEach((entry) => { if (!entry.isIntersecting) return; observer.unobserve(entry.target); callback(entry.target); }), {rootMargin: '500px'}); elements.forEach((element) => observer.observe(element)); }
  function renderProviderPill(container, provider, hidden = false) { const pill = document.createElement('span'); pill.className = `provider-pill${hidden ? ' d-none' : ''}`; pill.textContent = `${provider.name} · ${provider.access}`; if (hidden) pill.dataset.additionalProvider = ''; container.appendChild(pill); }
  function renderProviders(container, data) {
    const allRows = providerRows(data); const rows = allRows.slice(0, 2); rows.forEach((provider) => renderProviderPill(container, provider));
    const additionalRows = allRows.slice(rows.length); additionalRows.forEach((provider) => renderProviderPill(container, provider, true));
    if (additionalRows.length) {
      const more = document.createElement('button'); more.type = 'button'; more.className = 'provider-pill provider-pill-more'; more.textContent = `+${additionalRows.length} more`; more.setAttribute('aria-expanded', 'false');
      more.addEventListener('click', () => { const expanded = more.getAttribute('aria-expanded') === 'true'; container.querySelectorAll('[data-additional-provider]').forEach((pill) => pill.classList.toggle('d-none', expanded)); more.setAttribute('aria-expanded', expanded ? 'false' : 'true'); more.textContent = expanded ? `+${additionalRows.length} more` : 'Show fewer'; }); container.appendChild(more);
